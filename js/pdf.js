  ...
  generate(work) {
    try {
      ...
      const addRow = (label, value, bold = false, valueColor = null) => {
        ...
        doc.setTextColor(...(valueColor || [26, 26, 26]));
      ...
      if (work.breakMinutes > 0) {
        addRow('Break Time', fmtDuration(work.breakMinutes), false, [230, 81, 0]);
      }
      ...
      const pColor = work.paymentStatus === 'Paid'
        ? [46,125,50] : work.paymentStatus === 'Pending'
        ? [198,40,40] : [230,81,0];
      ...
      // ── Loading / Trips Section ───────────────────────────
      const loading = Array.isArray(work.loading) ? work.loading.filter(v => v.trips > 0) : [];
      if (loading.length) {
        // Check page overflow — add a new page if needed
        if (y > 230) { doc.addPage(); y = 20; }

        doc.setFont('helvetica', 'bold');
        doc.setFontSize(11);
        doc.setTextColor(26, 26, 26);
        doc.text('LOADING / TRIPS', margin, y);
        y += 6;
        doc.setDrawColor(224, 224, 224);
        doc.line(margin, y, W - margin, y);
        y += 8;

        const typeLabel = t => ({ Tractor:'Tractor', Lorry:'Lorry', Truck:'Truck' }[t] || 'Vehicle');
        loading.forEach(v => {
          doc.setFont('helvetica', 'normal');
          doc.setFontSize(10);
          doc.setTextColor(100, 100, 100);
          doc.text(`${typeLabel(v.vehicleType)} — ${v.vehicleNumber}`, margin, y);
          doc.setFont('helvetica', 'bold');
          doc.setTextColor(26, 26, 26);
          doc.text(`${v.trips} trips`, W - margin, y, { align: 'right' });
          y += 8;
        });
        ...
        doc.setTextColor(249, 196, 0);
        doc.text(String(totalTrips), W - margin, y, { align: 'right' });
        y += 12;
      }
